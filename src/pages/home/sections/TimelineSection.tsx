...
import { experienceMilestones } from "../data";
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-4 sm:px-6">
        <div className="mb-8 sm:mb-12 flex flex-col gap-4 sm:gap-8 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <div className="inline-flex items-center gap-2 sm:gap-3 rounded-full border border-primary/20 bg-primary/10 px-3 sm:px-5 py-1.5 sm:py-2">
              <IconBadge size="sm" />
              <span className="text-[10px] sm:text-sm font-semibold uppercase tracking-[0.15em] sm:tracking-[0.2em] text-primary">Timeline</span>
            </div>
            <h2 className="mt-4 sm:mt-6 text-xl sm:text-3xl md:text-4xl lg:text-5xl font-black">
              15 ans d'innovation continue.
            </h2>
          </div>
          <p className="max-w-xl text-xs sm:text-sm md:text-base text-muted-foreground">
            Chaque palier nous a rapprochés d'un objectif : offrir aux créateurs d'Algérie une infrastructure premium.
          </p>
        </div>
        ...
        <div className="relative pl-0 sm:pl-12">
          ...
          {experienceMilestones.map((milestone, index) => (
            <div key={milestone.year} className="relative pb-6 sm:pb-16 last:pb-0 sm:pl-2">
              ...
              <div className="rounded-2xl sm:rounded-3xl border border-border/40 bg-card/70 p-4 sm:p-6 backdrop-blur-xl shadow-[0_20px_45px_-25px_rgba(59,130,246,0.35)]">
                ...
                <h3 className="mt-2 sm:mt-3 text-base sm:text-lg md:text-xl font-bold text-foreground">{milestone.title}</h3>
                <p className="mt-2 sm:mt-3 text-xs sm:text-sm leading-relaxed text-muted-foreground">{milestone.description}</p>
              </div>
